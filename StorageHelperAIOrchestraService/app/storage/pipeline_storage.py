...
class PipelineStorage:
    ...
    def _convert_to_feature_format(
        ...
        def build_dishes_for_names(
            ...
            for dish_name in dish_names:
                ...
                # Preserve previously-saved cooking steps so a plan update
                # never wipes out steps that were generated by CookingStepsAgent.
                if existing_cooking_steps:
                    steps = existing_cooking_steps.get(cleaned)
                    if not steps:
                        # Fuzzy fallback: try partial name match
                        cl = cleaned.lower()
                        for k, v in existing_cooking_steps.items():
                            if cl in k.lower() or k.lower() in cl:
                                steps = v
                                break
                    if steps:
                        dish_obj["cookingSteps"] = steps
    ...
    def _extract_cooking_steps_from_schedule(
        ...
        """
        Walk a schedule's feature data and return a mapping of
        dish_name -> cookingSteps (list of step strings).
        Only dishes that actually have a non-empty cookingSteps field are included.
        """
            ...
            for plan in (feat.get("plans") or []):
                for meal in (plan.get("meals") or []):
                    for dish in (meal.get("dishes") or []):
                        ...
                        if name and steps and isinstance(steps, list):
                            result[name] = steps
        return result
    ...
    async def create_or_update_meal_plan_schedule(
        ...
        if meal_plan and len(meal_plan) > 1:
            ...
            for date_str, meal_text in meal_plan.items():
                try:
                    ...
                    # Preserve any cooking steps already saved for this date's dishes
                    existing_cs = self._extract_cooking_steps_from_schedule(existing_schedule) if existing_schedule else {}
                    metadata = self._convert_to_feature_format(
                        ...
                        existing_cooking_steps=existing_cs or None,
                    )
        ...
        # Merge dish_ingredients, meal_plan_slots, and cooking steps from existing schedule when updating by id
        merged_dish_ingredients = dish_ingredients
        merged_meal_plan_slots = meal_plan_slots
        merged_cooking_steps: Dict[str, List[str]] = {}
        if existing_schedule_id:
            ...
            for s in schedules:
                if s.get("id") == existing_schedule_id:
                    ...
                    # Preserve existing cooking steps so plan updates never wipe them out
                    merged_cooking_steps = self._extract_cooking_steps_from_schedule(s)
                    break
        ...
        metadata = self._convert_to_feature_format(
            ...
            existing_cooking_steps=merged_cooking_steps or None,
        )
        ...
        if draft:
            ...
            # Merge dish_ingredients, meal_plan_slots, and cooking steps from draft with passed
            existing_dish_ingredients = {}
            existing_slots = {}
            draft_cooking_steps: Dict[str, List[str]] = {}
            if draft.get("metadata") and isinstance(draft.get("metadata"), dict):
                ...
                draft_cooking_steps = self._extract_cooking_steps_from_schedule(draft)
            ...
            merged_metadata = self._convert_to_feature_format(
                ...
                existing_cooking_steps=draft_cooking_steps or None,
            )
